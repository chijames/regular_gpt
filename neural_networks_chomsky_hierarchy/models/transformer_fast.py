...
def compute_sliding_window_mask(sequence_length: int,
  ...
  attention_mask = jnp.sum(
      jnp.stack([
          ...
          for k in range(1, attention_window)
      ]),
      axis=0)
  attention_mask = attention_mask.at[::2].set(0)
  if not last_layer:
    attention_mask = attention_mask.at[-1,:].set(0)
  attention_mask = attention_mask + jnp.eye(sequence_length, sequence_length)
...
class TransformerLayer(hk.Module):
    ...
    def __call__(self, h, causal_mask, last_layer):
      # add positional embeddings to h
      front = hk.get_parameter('front', (self._config.embedding_dim,), init=functools.partial(jrandom.normal, hk.next_rng_key()))
      rear = hk.get_parameter('rear', (self._config.embedding_dim,), init=functools.partial(jrandom.normal, hk.next_rng_key()))
      h = h.at[:, :-1:2].add(front)
      h = h.at[:, 1:-1:2].add(rear)
      ...
      indices = list(range(1, sequence_length-1, 2))
      if sequence_length % 2 == 0 and not last_layer:
        indices.append(sequence_length-2)
...
class MultiHeadDotProductAttention(hk.Module):
  ...
  def __call__(
    ...
    if self._positional_encodings == pos_encs_lib.PositionalEncodings.RELATIVE:
      ...
      q_indices = list(range(1, sequence_length-1, 2))
      if sequence_length % 2 == 0 and not last_layer:
        q_indices.append(sequence_length-2)
      q_indices.append(sequence_length-1)
      # every q vector will attend to 2 preceding k vectors
      # we will mask out some of the attention weights later on
      q = q[:, q_indices]
      #q = jnp.reshape(q, (batch_size, -1, 1, self._num_heads, self._num_hiddens_per_head))
      kv_indices = []
      for qi in q_indices:
        kv_indices += [qi-1, qi]
      k = k[:, kv_indices]
      v = v[:, kv_indices]
      k = jnp.reshape(k, (batch_size, -1, 2, self._num_heads, self._num_hiddens_per_head))
      v = jnp.reshape(v, (batch_size, -1, 2, self._num_heads, self._num_hiddens_per_head))
      #q = jnp.einsum('bgqhd->bhgqd', q)
      #k = jnp.einsum('bgkhd->bhgdk', k)
      attention = jnp.einsum('bghd,bgkhd->bhgk', q, k) # [bs, num_heads, num_groups, 2]
    attention *= 1. / jnp.sqrt(self._num_hiddens_per_head)
    # post processing
    if not last_layer:
      attention = attention.at[:, :, -1, 0].set(jnp.finfo(jnp.float32).min)
      if (sequence_length - 1) % 2 == 1:
        attention = attention.at[:, :, -2, 0].set(jnp.finfo(jnp.float32).min)
    else:
    ...
    output = jnp.einsum('bhgk,bgkhd->bghd', normalized_attention, v)
    output = jnp.reshape(output, (batch_size, len(q_indices), num_hiddens))
...
class TransformerEncoder(hk.Module):
  ...
  def __call__(self, x: jnp.ndarray) -> chex.Array:
    ...
    dynamic_num_layers = int(np.ceil(np.log2(sequence_length))) + 1
    for cur_layer in range(dynamic_num_layers):
      if not self._config.share_weight or cur_layer == 0:
        transformer_layer = TransformerLayer(self._config, pos_enc_params)
      h = transformer_layer(h, causal_mask, cur_layer==dynamic_num_layers-1)
    return h
